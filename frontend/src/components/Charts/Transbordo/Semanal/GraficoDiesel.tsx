...
const META_DIESEL = configManager.getMetas('transbordo_semanal').diesel;

// Dados de exemplo para o caso de não serem fornecidos
const defaultData: DataItem[] = [
  { frota: '6031', valor: 8.5 },
  { frota: '6082', valor: 8.9 }
];

export const GraficoDiesel: React.FC<GraficoDieselProps> = ({ 
  data = defaultData,
  meta = META_DIESEL
}) => {
          ...
          <Box key={index} w="100%">
            <Flex justify="space-between" mb={1}>
              ...
              <Text fontSize="11px" fontWeight="bold" color={getBarColor(item?.valor || 0)}>
                {formatarValor(item?.valor || 0)} L/h
              </Text>
            </Flex>
            ...
            <Flex mt={1} justify="space-between">
              ...
              <Text fontSize="9px" color="gray.500">
                Meta: {formatarValor(meta)} L/h
              </Text>
              <Text fontSize="9px" color="gray.500">{formatarValor(escala)} L/h</Text>
            </Flex>
          </Box>
        ))}
      </VStack>
